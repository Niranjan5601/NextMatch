...
import { sendPaswwordResetEmail, sendVerificationEmail } from '@/lib/mail';

export async function registerUser(data: RegisterSchema): Promise<ActionResult<User>> {
    try {

        const validated = combinedRegisterSchmea.safeParse(data);
...
export async function signInUser(data: LoginSchema): Promise<ActionResult<string>> {
    try {
        ...
        if (!existingUser.emailVerified) {
            ...
            return { status: 'error', error: 'Please verify you email address before logging in' }
        }
...
export async function verifyEmail(token: string): Promise<ActionResult<string>> {
    try {
        ...
        const exitsingUser = await getUserByEmail(existingToken.email);
        if (!exitsingUser) {
            return { status: 'error', error: 'User not found' };
        }

        await prisma.user.update({
            where: { id: exitsingUser.id },
...
export async function generateResetPasswordEmail(email:string):Promise<ActionResult<string>>{
    try {
        ...
        await sendPaswwordResetEmail(token.email,token.token);
        return {status:'success', data:'Password reset email has been sent. Please check your mail'}
    } catch (error) {
        console.log(error);
        return {status:'error',error:'Something went wrong'}
    }
